...
# ТВОЙ ПРОГНОЗ 6.1:
# Обращение к несуществующему ключу словаря ({"a": 1}["b"]) —
# это классическая ситуация "ключ отсутствует в словаре".
# Реальный тип исключения: KeyError
# (сообщение будет примерно: KeyError: 'b')

try:
    {"a": 1}["b"]
except Exception as e:
    print(f"6.1 Реальный тип: {type(e).__name__}, сообщение: {e}")

# ТВОЙ КОД ЗДЕСЬ:
# KeyError — это подкласс Exception (KeyError наследуется от Exception
# через LookupError). А except ловит не только точный класс, но и всех
# его потомков — как обычная проверка isinstance. Поэтому except Exception
# ловит вообще всё, кроме совсем системных вещей типа SystemExit.
# Из-за этого такой except — плохая практика: он молча глотает любые
# ошибки подряд, даже те, которые ты не предвидел и не хотел скрывать.
