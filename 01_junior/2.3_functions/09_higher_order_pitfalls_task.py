...
print("\n" + "=" * 60)
print(
    "ЗАДАНИЕ 4: map() с ДВУМЯ итерируемыми — когда lambda с двумя "
    "параметрами уместна"
)
print("=" * 60)
print("""
...
ids: list[int] = [1, 2, 3]
balances: list[int] = [-5, 3, 8]

# ТВОЁ ОБЪЯСНЕНИЕ 4.2:
# map(func, list1, list2) вызывает func с двумя отдельными аргументами на каждой итерации (по одному элементу из каждого списка одновременно, func(a, b)), тогда как map(func, list_of_pairs) вызывает func с одним аргументом — целой парой (кортежем/списком) как единым объектом (func((a, b))), и если func ожидает два раздельных параметра, это приведёт к TypeError из-за несовпадения количества аргументов (если только func не распаковывает пару вручную внутри себя).
# ТВОЙ КОД ЗДЕСЬ:
strings = list(map(lambda a, b: f"{a}:{b}", ids, balances))
...
print("\n" + "=" * 60)
print("КОНЕЦ. Правило на будущее: каждый прогноз — ДО запуска, не после.")
print("=" * 60)
