...
# Если программа упадёт между двумя commit(), деньги у from_id уже
# списаны и зафиксированы, а к to_id ещё не зачислены — деньги
# просто исчезают, и баланс системы нарушается.
# Одна транзакция с одним commit() даёт атомарность: либо оба UPDATE
# фиксируются вместе, либо при сбое до commit() ничего не применяется
# и перевода как будто не было.
