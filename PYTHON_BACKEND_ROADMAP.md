...
> **Прогресс:** Junior Level, Блок 2.3 завершён (ждёт финальную оценку); Блок 2.7 (Ошибки и исключения), тема 1 (try/except основы) завершена 8/8 — ВНЕ ОЧЕРЕДИ, до 2.4/2.5/2.6; блок на паузе, дальше идёт свободная практика вне роудмапа
...
### Блок 2.4. Модули, файлы и дата/время (1–2 недели)

- [ ] Модули: `import`, `from ... import`, `as`; что происходит при импорте своего модуля
- [ ] `if __name__ == "__main__"` — зачем нужен, разница между запуском файла и импортом
- [ ] Пакеты: `__init__.py`, абсолютные vs относительные импорты, структура проекта
- [ ] Поиск модулей: `sys.path`, порядок поиска (обзорно)
...
- [ ] Дата и время: `datetime`, `date`, `time`, `timedelta`
- [ ] Форматирование дат: `strftime`/`strptime`, ISO-формат
- [ ] Часовые пояса — обзорно (`zoneinfo`)

### Блок 2.5. ООП и модель данных (3 недели)
