...
### Блок 2.1. Фундамент языка (3–4 недели) 🔄 В ПРОЦЕССЕ (~100% + дополнено)
...
> **Статус:** Все 7 тем закрыты (10.06.2026). **Переменные выделены в отдельный файл (15.06.2026)**.
> Ожидает финальная оценка блока 2.1 (11.06.2026). После ≥80% — переход на блок 2.2.

### Блок 2.2. Структуры данных (2–3 недели) 🔄 В ПРОЦЕССЕ

- [x] Теория: `list`, `tuple`, `dict`, `set` (основные операции)
- [x] List comprehensions (базовое понимание)
- [x] Выбор структуры под задачу (8/8 микровопросов пройдено)
- [ ] `list`: срезы, методы, сортировка с `key`, list comprehensions (финальная оценка)
- [ ] `tuple`, распаковка, именованные кортежи (`collections.namedtuple`, `typing.NamedTuple`)
- [ ] `dict`: методы, dict comprehensions, `.get`/`.setdefault`, порядок вставки (3.7+)
- [ ] `set` / `frozenset`: операции над множествами, дедупликация
- [ ] `collections`: `defaultdict`, `Counter`, `deque`, `OrderedDict`
- [ ] Генераторные выражения vs списки — память и ленивость
- [ ] Выбор структуры под задачу: сложность операций (O-нотация на пальцах)
