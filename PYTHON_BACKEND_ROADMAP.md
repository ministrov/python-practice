...
> **Старт:** 2026-06-06
> **Последний чекпойнт:** 2026-07-24 (сеанс 20)
> **Прогресс:** Junior Level, Блоки 2.1–2.4 полностью завершены (включая финальные оценки 2.2 и 2.3, 100% каждая); Блок 2.5 (ООП) в процессе — темы 1-6 завершены 8/8 (тема 6 dunder-методы — 2026-08-11), тема 7 (абстракция, `abc.ABC`) следующая; Блок 2.7 (Ошибки и исключения), тема 1 (try/except основы) завершена 8/8 — ВНЕ ОЧЕРЕДИ, на паузе, темы 2-5 остаются последним долгом; Блок 2.6 не начат
...
### Блок 2.5. ООП и модель данных (3 недели) 🔄 В ПРОЦЕССЕ (темы 1-6 завершены, последняя — 2026-08-11; долги по 2.2/2.3/2.4 закрыты 2026-07-24, остаётся долг по 2.7)
...
- [ ] Абстракция: `abc.ABC`, `@abstractmethod` — формальный запрет создавать
      экземпляр базового класса, пока наследник не реализует контракт
      (материалы готовы: `13_abstraction_demo.py`, `14_abstraction_task.py`,
      2026-08-11)
...
- [ ] Обобщение: 4 столпа ООП (инкапсуляция/абстракция/наследование/
      ...
      было сделано практически в темах 2-7 (property → инкапсуляция,
