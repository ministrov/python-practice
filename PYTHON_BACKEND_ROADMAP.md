...
> **Старт:** 2026-06-06
> **Последний чекпойнт:** 2026-08-19
> **Прогресс:** Junior Level, Блоки 2.1–2.6 полностью завершены (включая финальные оценки 2.2, 2.3 и 2.5, 100% каждая; 2.6 темы 3-5 ознакомительно); Блок 2.7 (Ошибки и исключения) — темы 1-4 завершены (тема 2 полной практикой 7/7, темы 3-4 ознакомительно), тема 5 (logging) — последняя, ещё не начата
...
- [x] **Финальная оценка блока 2.2** — ✅ пройдена 2026-07-24, 100% (`assessments/ASSESSMENT_Block_2_2.md`)
...
### Блок 2.3. Функции (2–3 недели) ✅ ЗАВЕРШЕН (финальная оценка пройдена 2026-07-24, 100%)
...
- [x] **Финальная оценка блока 2.3** — ✅ пройдена 2026-07-24, 100% (`assessments/ASSESSMENT_Block_2_3.md`)

### Блок 2.4. Модули, файлы и дата/время (1–2 недели) ✅ ЗАВЕРШЕН (все 3 темы, 2026-07-24)
...
- [x] Работа с файлами продвинуто: `pathlib.Path` вместо строк-путей, обход директорий ✅ (2026-07-24)
  - Файлы: `03_files_advanced_demo.py` ✅, `04_files_advanced_task.py` ✅ (8/8 заданий)
- [x] Структурированные форматы: `csv`, `json` — чтение и запись ✅ (2026-07-24, в теме выше)
- [x] Бинарные файлы, кодировки (`encoding=`), типичные ошибки (`UnicodeDecodeError`) ✅ (2026-07-24, в теме выше)
...
### Блок 2.5. ООП и модель данных (3 недели) ✅ ЗАВЕРШЕН (все 9 тем + финальная оценка пройдена 2026-08-14, 100%); долги по 2.2/2.3/2.4 закрыты 2026-07-24
...
- [x] Наследование, `super()`, MRO (method resolution order) поверхностно (2026-07-27)
- [x] Композиция vs наследование — концепция (2026-07-28)
- [x] `dataclasses` — основной инструмент для классов-данных (2026-08-10)
- [x] Dunder-методы: `__repr__`, `__str__`, `__eq__`, `__hash__`, `__len__`, `__iter__` (2026-08-11)
- [x] Абстракция: `abc.ABC`, `@abstractmethod` — формальный запрет создавать
      экземпляр базового класса, пока наследник не реализует контракт
      (`13_abstraction_demo.py`, `14_abstraction_task.py`, 8/8 заданий,
      2026-08-13)
- [x] `Enum`, `IntEnum` (`StrEnum` не покрывался отдельно — не критично для
      уровня блока) (`15_enum_demo.py`, `16_enum_task.py`, 7/7 заданий,
      2026-08-13)
- [x] Обобщение: 4 столпа ООП (инкапсуляция/абстракция/наследование/
      полиморфизм) — итоговая тема блока, назвала словами то, что уже
      было сделано практически в темах 2-8 (property → инкапсуляция,
      override + dunder → полиморфизм, abc.ABC → абстракция)
      (`17_four_pillars_demo.py`, `18_four_pillars_task.py`, 5/5
      заданий, 2026-08-13)
- [x] **Финальная оценка блока 2.5** — ✅ пройдена 2026-08-14, 100%
      (8/8 микровопросов + 7/7 практика "Библиотечная система";
      `assessments/ASSESSMENT_Block_2_5.md`)

### Блок 2.6. Типизация и качество кода (2 недели) ✅ ЗАВЕРШЁН (2026-08-14 → 2026-08-18)
...
- [x] Проверка типов: `mypy` или `pyright` — настройка и запуск ✅ (2026-08-18, ознакомительно)
  - Файл: `05_type_checking_setup_demo.py` ✅ (теория без практики — решение пользователя)
- [x] PEP 8, форматирование: `ruff` / `black`, линтинг `ruff` ✅ (2026-08-18, ознакомительно)
  - Файл: `07_pep8_formatting_linting_demo.py` ✅ (теория без практики)
- [x] Виртуальные окружения: `venv`, управление зависимостями (`pip`, `requirements.txt`, или `uv`/`poetry`) ✅ (2026-08-18, ознакомительно)
  - Файл: `08_venv_pip_requirements_demo.py` ✅ (теория без практики)

### Блок 2.7. Ошибки, исключения, стандартная библиотека (2 недели) 🔄 В ПРОЦЕССЕ (тема 1-4 завершены, тема 5 — последняя)
...
- [x] `raise`, `raise from`, exception chaining; `ExceptionGroup`/`except*` (3.11+) ✅ (2026-08-19)
  - Файлы: `03_raise_from_demo.py` ✅, `04_raise_from_task.py` ✅ (7/7 заданий)
- [x] Контекстные менеджеры: `with`, `contextlib.contextmanager` ✅ (2026-08-19, ознакомительно)
  - Файл: `05_context_managers_demo.py` ✅ (теория без практики — решение пользователя)
- [x] Полезное из stdlib: `json`, `re`, `os`/`sys`, `itertools`, `functools` (`pathlib`/`datetime` — см. Блок 2.4) ✅ (2026-08-19, ознакомительно)
  - Файл: `06_stdlib_demo.py` ✅ (теория без практики)
- [ ] `logging` базово: уровни, форматтеры, хендлеры (вместо `print`) — ознакомительно, следующая тема
