...
- [x] Оценочное тестирование: микровопросы 8/8 + практическая задача 6/6 ✅

> **Статус:** ✅ ПОЛНОСТЬЮ ЗАВЕРШЕН (18.06.2026)
> - Все 7 тем закрыты (10.06.2026)
> - Переменные выделены в отдельный файл (15.06.2026)
> - Финальная оценка пройдена (микровопросы 100%, практика 100%)
> - Результат: Grade Pass ✅

### Блок 2.2. Структуры данных (2–3 недели) 🔄 В ПРОЦЕССЕ (сеанс 7: 2026-06-18)

#### ✅ Завершено
- [x] Тема 1: **List (список)** — `01_list_demo.py` + `02_list_task.py` (8/8 заданий решены) ✅ 2026-06-18
  - Создание, индексирование, срезы, методы (append, insert, remove, pop, extend, clear)
  - len(), count(), index(), сортировка (sort, sorted), проверка принадлежности (in)
  - Итерация и изменяемость списков
  
- [x] Тема 2: **Tuple (кортеж)** — `03_tuple_demo.py` + `04_tuple_task.py` (8/8 заданий решены) ✅ 2026-06-18
  - Создание, неизменяемость, распаковка (unpacking), использование как ключ словаря
  - Методы: count(), index(), операции конкатенации (+, *)
  - Named tuples (collections.namedtuple)

#### 🔄 В процессе
- [ ] Тема 3: **Dict (словарь)** — `05_dict_demo.py` готов + `06_dict_task.py` готов (8 заданий для решения)
  - Доступ ([], .get()), добавление, изменение, удаление (del, .pop())
  - Методы: .keys(), .values(), .items(), .update(), .setdefault()
  - Вложенные словари, dict comprehensions
  
- [ ] Тема 4: **Set (множество)** — файлы готовятся (8 заданий)
  - Создание, операции над множествами, дедупликация, frozenset
  
- [ ] Тема 5: **List Comprehensions** — файлы готовятся (8 заданий)
  - List, dict, set comprehensions, генераторные выражения
  
- [ ] Тема 6: **Выбор подходящей структуры** — файлы готовятся (8 микровопросов)
  - Сложность операций, когда какую структуру использовать
  
- [ ] `collections`: `defaultdict`, `Counter`, `deque`, `OrderedDict`
- [ ] Финальная оценка Блока 2.2 (4 комплексных задания, критерий ≥80%)

### Блок 2.3. Функции (2–3 недели)

- [ ] Объявление, позиционные и именованные аргументы, значения по умолчанию (ловушка mutable default!)
...
- [ ] Декораторы: базовое применение, `functools.wraps`, декораторы с аргументами
- [ ] Распаковка аргументов, аннотации типов (введение)

### Блок 2.4. ООП и модель данных (3 недели)

- [ ] Классы, `__init__`, атрибуты экземпляра vs класса, `self`
- [ ] Методы: обычные, `@classmethod`, `@staticmethod`, `@property` (геттеры/сеттеры)
...
### Блок 2.5. Типизация и качество кода (2 недели)
...
- [ ] Структура проекта, `__init__.py`, импорты (абсолютные vs относительные)

### Блок 2.6. Ошибки, исключения, стандартная библиотека (2 недели)

- [ ] `try/except/else/finally`, иерархия исключений, свои классы исключений
- [ ] `raise`, `raise from`, exception chaining; `ExceptionGroup`/`except*` (3.11+)
- [ ] Контекстные менеджеры: `with`, `contextlib.contextmanager`
- [ ] Полезное из stdlib: `pathlib`, `datetime`, `json`, `re`, `os`/`sys`, `logging`, `itertools`, `functools`
- [ ] `logging` базово: уровни, форматтеры, хендлеры (вместо `print`)

### Блок 2.7. Базы данных и SQL — основы (3 недели)
...
### Блок 2.8. Введение во фреймворк — FastAPI (3–4 недели)
...
### Блок 2.9. Git и инструменты (2 недели, параллельно)
...
- [ ] Все блоки 2.1–2.9 пройдены (≥ 80% галочек)
